...
class IndicatorIconManager implements ManageAble {
  ...
  IndicatorIconManager(List<IconData> iconData,
      {required double size,
      required Color color,
      EdgeInsets? padding,
      required int period,
      required int Function(int, ManageAble) getState,
      void Function(int, ManageAble)? onClick,
      bool enabled = true,
      bool visible = true}) {
    key = GlobalKey();
...
class _IndicatorIcon extends StatefulWidget {
  ...
  const _IndicatorIcon(this._iconData,
      {super.key,
      required this.size,
      required this.color,
      this.padding,
...
class _IndicatorIconState extends State<_IndicatorIcon> implements ManageAble {
  ...
  initState() {
    ...
    if (_icons.isEmpty) {
      if (widget.size > 2) {
        _icons.add(Icon(Icons.check_box_outline_blank,
    ...
    if (_icons.length == 1) {
      if (widget.size > 2) {
        _icons.add(Icon(Icons.indeterminate_check_box_outlined,
            color: widget.color, size: widget.size));
      } else {
        _icons.add(
            Icon(Icons.indeterminate_check_box_outlined, color: widget.color));
      }
    }

    _timer =
        Timer.periodic(Duration(milliseconds: widget.period), (timer) async {
      if (mounted) {
        if (_enabled) {
...
class DetailIconButtonManager implements ManageAble {
  late final GlobalKey key;
  late final Widget widget;
  DetailIconButtonManager(
      {bool visible = true,
      bool enabled = true,
      int timerMs = clickTimerMs,
      required IconData iconData,
      String tooltip = "",
      required AppThemeData appThemeData,
      EdgeInsetsGeometry? padding,
      required Function(ManageAble) onPressed}) {
    key = GlobalKey();
    widget = DetailIconButton(
        key: key,
        visible: visible,
        enabled: enabled,
        timerMs: timerMs,
    ...
    if (_visible) {
      return IconButton(
        ...
        constraints: BoxConstraints.tightFor(
            width: iSize + widget.appThemeData.iconGap, height: iSize),
        icon: Icon(widget.iconData,
            size: iSize,
            color:
                widget.appThemeData.screenForegroundColour(_enabled && !_grey)),
        onPressed: () {
          if (!_enabled || _grey) {
            return;
          }
...
class _DetailTextButtonState extends State<DetailTextButton>
    implements ManageAble {
  bool _grey = false;
  late bool _enabled;
  late bool _visible;
  ...
  Widget build(BuildContext context) {
    if (_visible) {
      final fg = widget.appThemeData.screenForegroundColour(_enabled && !_grey);
      final style = TextButton.styleFrom(
        foregroundColor: fg,
        shape: RoundedRectangleBorder(
          side: BorderSide(color: fg, width: 2),
          borderRadius: widget.appThemeData.borderRadius,
        ),
      );
       final button = SizedBox(
        ...
        child: TextButton(
          ...
          child: Text(widget.text,
              style: (_enabled && !_grey)
                  ? widget.appThemeData.tsMediumBold
...
Widget inputTextField(final TextStyle ts, final TextSelectionThemeData theme,
    final bool isDarkMode, final TextEditingController controller,
    {final double width = 0,
    final double height = 0,
    final bool isPw = false,
    final String hint = "",
    final EdgeInsetsGeometry padding = const EdgeInsets.fromLTRB(5, 5, 0, 0),
    final FocusNode? focusNode,
    final bool autoFocus = true,
    required final Function(String)? onChange,
    required final Function(String)? onSubmit}) {
  return SizedBox(
...
void markdownOnTapLink(String text, String? href, String title,
    Path Function(DetailAction) dataAction) {
  if (href != null) {
...
Widget markdownDisplayWidget(bool show, String text, Color background,
    void Function(String, String?, String) onTapLinkExt,
            ...
            child: Column(
              children: [
                markdownDisplayWidget(
                    ...
                    controller.text,
                    widget.appThemeData.secondary.light, (text, href, title) {
                  markdownOnTapLink(text, href, title, widget.dataAction);
                }),
                markdownDisplayWidget(widget.shouldDisplayHelp(false), helpText,
                    widget.appThemeData.hiLight.light, (text, href, title) {
                  markdownOnTapLink(text, href, title, widget.dataAction);
                }),
                Expanded(
                  child: TextField(
                    ...
                    onChanged: (value) {
                      setState(() {
                        widget.onValidate(widget.initialText, controller.text,
                            optionTypeDataString, optionTypeDataString);
                      });
                    },
                  ),
                ),
...
class ValidatedInputField extends StatefulWidget {
  ValidatedInputField(
      ...
      this.options = const [],
      this.initialOption = optionsDataTypeEmpty,
      required this.appThemeData});
...
class _ValidatedInputFieldState extends State<ValidatedInputField> {
  ...
  initState() {
    ...
    validateResponse =
        widget.onValidate(initial, current, initialOption, currentOption);
  ...
  void _validate() {
    setState(() {
      validateResponse = widget.onValidate(
          initial, current.trim(), initialOption, currentOption);
    });
  }
                ...
                children: [
                  IconButton(
                      ...
                      icon: Icon(
                          ...
                          size: widget.appThemeData.iconSize)),
